...
// Limite máximo de jogadores por sala
	...
	socket.on("message", (message) => {
		...
		switch (data.cmd) {
			case "create_room": {
				...
				// Inicia a partida imediatamente com 1 jogador
				socket.send(
					JSON.stringify({
						cmd: "start_game",
						content: {},
			...
			case "join_room": {
				...
				// Começa o jogo imediatamente quando houver pelo menos 1 jogador
				if (Object.keys(roomToJoin.players).length >= 1) {
					console.log(
						`Sala ${socket.roomId} ativa com ${Object.keys(roomToJoin.players).length} jogador(es).`
					);
